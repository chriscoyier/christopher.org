...
class Smart_Quotes_Fix extends Abstract_Node_Fix
{
    ...
    public function apply(\DOMText $textnode, Settings $settings, $is_title = \false)
    {
        ...
        if ($double != $this->cached_primary_quotes || $single != $this->cached_secondary_quotes) {
            // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison -- object value comparison.
        ...
        $node_data = \preg_replace([self::SINGLE_QUOTED_NUMBERS . $f['u'], self::DOUBLE_QUOTED_NUMBERS . $f['u']], ["{$single_open}\$1{$single_close}", "{$double_open}\$1{$double_close}"], $node_data);
        // Guillemets.
        $node_data = \str_replace(['<<', '>>'], [U::GUILLEMET_OPEN, U::GUILLEMET_CLOSE], $node_data);
        // Primes.
        $node_data = \preg_replace([
            self::SINGLE_DOUBLE_PRIME . $f['u'],
            self::DOUBLE_PRIME . $f['u'],
            // should not interfere with regular quote matching.
        ...
        $node_data = \str_replace(['``', '`', "''", ',,'], [$double_open, $single_open, $double_close, U::DOUBLE_LOW_9_QUOTE], $node_data);
        $node_data = \preg_replace(self::COMMA_QUOTE . $f['u'], U::SINGLE_LOW_9_QUOTE, $node_data);
        // like _,¿hola?'_.
        // Apostrophes.
        $node_data = \preg_replace([self::APOSTROPHE_WORDS . $f['u'], self::APOSTROPHE_DECADES . $f['u']], [U::APOSTROPHE, U::APOSTROPHE . '$1'], $node_data);
        // Quotes.
        $node_data = \str_replace($this->brackets_matches, $this->brackets_replacements, $node_data);
        $node_data = \preg_replace([self::SINGLE_QUOTE_OPEN . $f['u'], self::SINGLE_QUOTE_CLOSE . $f['u'], self::DOUBLE_QUOTE_OPEN . $f['u'], self::DOUBLE_QUOTE_CLOSE . $f['u']], [$single_open, $single_close, $double_open, $double_close], $node_data);
        ...
        $node_data = \str_replace(["{$double_open}{$single_open}", "{$single_close}{$double_close}"], [$double_open . U::NO_BREAK_NARROW_SPACE . $single_open, $single_close . U::NO_BREAK_NARROW_SPACE . $double_close], $node_data);
        ...
        $next_length = self::calc_adjacent_length($f['strlen']($next_character), $next_character, $node_data, $quotes, $f['substr'], $f['strlen'], \true);
