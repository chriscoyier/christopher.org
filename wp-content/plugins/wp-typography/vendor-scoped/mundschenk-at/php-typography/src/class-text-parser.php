...
class Text_Parser
{
    const NO_ALL_LETTERS = 0b1;
    const ALLOW_ALL_LETTERS = 0b10;
    const REQUIRE_ALL_LETTERS = 0b100;
    const NO_ALL_CAPS = 0b1000;
    const ALLOW_ALL_CAPS = 0b10000;
    const REQUIRE_ALL_CAPS = 0b100000;
    const NO_COMPOUNDS = 0b1000000;
    const ALLOW_COMPOUNDS = 0b10000000;
    const REQUIRE_COMPOUNDS = 0b100000000;
    /**
    ...
    private const SPACE = '(?:\s|' . self::HTML_SPACING . ')+';
    // required modifiers: x (multiline pattern) i (case insensitive) $utf8.
    /**
    ...
    // Find any text.
    private const ANY_TEXT = self::SPACE . '|' . self::PUNCTUATION . '|' . self::WORD;
    // required modifiers: x (multiline pattern) i (case insensitive) u (utf8).
    // Regular expressions.
    ...
    public function load($raw_text)
    {
        ...
        if (empty($str_functions)) {
            // TODO: Refactor encoding check.
            return \false;
            // unknown encoding.
        }
        $this->current_strtoupper = $str_functions['strtoupper'];
        // Tokenize the raw text parts.
        $this->text = self::tokenize(\preg_split(self::RE_ANY_TEXT, $raw_text, -1, \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_NO_EMPTY) ?: []);
        // phpcs:ignore WordPress.PHP.DisallowShortTernary.Found -- Ensure array type in case of error.
    ...
    public function get_words($abc = self::ALLOW_ALL_LETTERS, $caps = self::ALLOW_ALL_CAPS, $comps = self::ALLOW_COMPOUNDS)
    {
        ...
        foreach ($this->get_type(Token::WORD) as $index => $token) {
            if ($this->conforms_to_letters_policy($token, $abc) && $this->conforms_to_caps_policy($token, $caps) && $this->conforms_to_compounds_policy($token, $comps)) {
                $tokens[$index] = $token;
            }
        }
        return $tokens;
    ...
    protected function check_policy(Token $token, $policy, $permissive_policy, $equal_policy, $non_equal_policy, callable $transform_token)
    {
        ...
        return $equal_policy === $policy && $transformed === $token->value || $non_equal_policy === $policy && $transformed !== $token->value;
    }
    /**
 ...
 * If multibyte characters are passed, they must be encoded as UTF-8.
 */
\class_alias('WP_Typography\Vendor\PHP_Typography\Text_Parser', 'PHP_Typography\Text_Parser', \false);
